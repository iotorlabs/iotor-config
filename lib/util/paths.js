...
// Fallbacks for other operating systems
  ...
  paths.config = paths.config || path.join(base, '.config/racoon');
  paths.data = paths.data || path.join(base, '.local/share/racoon');
  paths.cache = paths.cache || path.join(base, '.cache/racoon');
}

paths.tmp = path.resolve(path.join(tmp, 'racoon'));
