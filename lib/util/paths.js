...
// Fallbacks for other operating systems
  ...
  paths.config = paths.config || path.join(base, '.config/ano');
  paths.data = paths.data || path.join(base, '.local/share/ano');
  paths.cache = paths.cache || path.join(base, '.cache/ano');
}

paths.tmp = path.resolve(path.join(tmp, 'ano'));
