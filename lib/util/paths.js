...
// Fallbacks for other operating systems
  ...
  paths.config = paths.config || path.join(base, '.config/iotor');
  paths.data = paths.data || path.join(base, '.local/share/iotor');
  paths.cache = paths.cache || path.join(base, '.cache/iotor');
}

paths.tmp = path.resolve(path.join(tmp, 'iotor'));
